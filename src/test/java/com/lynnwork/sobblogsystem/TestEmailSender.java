...
public class TestEmailSender {
    ...
    private TestEmailSender() {
        TestEmailSender.config(TestEmailSender.SMTP_163(false), "13788809958@163.com", "ZMskRHmAUYJtKv8q");
    }
     ...
     */
    public static TestEmailSender subject(String subject) {
        TestEmailSender TestEmailSender = new TestEmailSender();
        TestEmailSender.msg = new MimeMessage(session);
        try {
            TestEmailSender.msg.setSubject(subject, "UTF-8");
        } catch (Exception e) {
            log.info(TAG, e + "");
        }
     ...
     */
    public TestEmailSender from(String nickName) {
     ...
     */
    public TestEmailSender from(String nickName, String from) {
        try {
    ...
    public TestEmailSender replyTo(String... replyTo) {
    ...
    public TestEmailSender to(String... to) throws MessagingException {
    ...
    public TestEmailSender cc(String... cc) throws MessagingException {
    ...
    public TestEmailSender bcc(String... bcc) throws MessagingException {
    ...
    private TestEmailSender addRecipients(String[] recipients, Message.RecipientType type) throws MessagingException {
        String result = Arrays.asList(recipients).toString().replace("(^\\[|\\]$)", "").replace(", ", ",");
        msg.setRecipients(type, InternetAddress.parse(result));
        return this;
    }

    private TestEmailSender addRecipient(String recipient, Message.RecipientType type) throws MessagingException {
        msg.setRecipients(type, InternetAddress.parse(recipient.replace(";", ",")));
        return this;
    }

    public TestEmailSender text(String text) {
        this.text = text;
        return this;
    }

    public TestEmailSender html(String html) {
        this.html = html;
        return this;
    }

    public TestEmailSender attach(File file) {
        attachments.add(createAttachment(file, null));
        return this;
    }

    public TestEmailSender attach(File file, String fileName) {
        attachments.add(createAttachment(file, fileName));
        return this;
    }

    public TestEmailSender attachURL(URL url, String fileName) {
    ...
    public static void main(String[] args) throws MessagingException {
        TestEmailSender.subject("测试邮件发送")
                .from("阳光沙滩博客系统")
                .text("这是发送的内容：ab12rf")
                .to("903586678@qq.com")
                .send();
